...
def venta():
  while True:
    ...
    #temporal5=pd.read_csv('BDATOS/temporal2.csv')  # Para leer la base de datos temporal
    #temporal55=pd.DataFrame(temporal5)
    #temporal55=temporal55.reset_index(drop=True)
    #temporal55.to_csv('BDATOS/temporal2.csv', mode="w", index="", header="True")   # SE gaba las modifificaciones en la base de datos Temporal2
    ...
    #temp=pd.read_csv("BDATOS/temporal2.csv")          # Se lee la base de datos de temporal
    #temp1=pd.DataFrame()
    #temp1=pd.DataFrame.transpose(temp1)
    # Se coloca los nombres a las columnas
    #temp1.columns=['Cedula','Referencia','Nproducto','Punitario','Cantidad','Subtotal','Fecha','Hora']
    #temp1.to_csv('BDATOS/temporal2.csv', mode="w", index="", header="True")   # SE gaba las modifificaciones en la base de datos Temporal2
         ...
         # Nueva cantidad de unidades del producto para mostrador
          ...
          for TM in range(1):
            ...
            if (mm2.iloc[TM, 1]==Ref1):# Se crea un filtro para eliger la referencia correcta
               ...
               mm2.to_csv('BDATOS/mostrador.csv', mode="w", index="", header="") # se graba los nuevos datos en la base de datos de mostrador
    ...
    tempL=len(temporal22)
    print(tempL)
    LL=tempL
    temporal22.drop([LL],axis=0)
    temporal.to_csv('BDATOS/temporal.csv', mode="w", index="", header="True")
    temporal22=pd.read_csv("BDATOS/temporal.csv")
    print(temporal22)
...
def Factura():
  while True:
    ...
    #ventas2.to_csv('BDATOS/ventas.csv', mode="a", index="", header="True") # Se graba los datos para ventas totales en el archivo ventas.csv
